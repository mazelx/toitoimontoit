...
class LeBonCoinSearch(Search):
    def __init__(self, parameters, proxies):
        super().__init__(parameters, proxies)
        self.header = {'User-Agent': 'fr.leboncoin.android , Sony D5803 , 6.0.1',
    ...
    def search(self):
        ...
        if self.parameters.get('price'):
            payload['mrs'] = self.parameters['price'][0]  # Loyer min
            payload['mre'] = self.parameters['price'][1]  # Loyer max

        if self.parameters.get('surface'):
            payload['sqs'] = self.surface_value(self.parameters['surface'][0])  # Surface min
            payload['sqe'] = self.surface_value(self.parameters['surface'][1])  # Surface max

        if self.parameters.get('rooms'):
            payload['ros'] = self.parameters['rooms'][0]  # Pièces min
            payload['roe'] = self.parameters['rooms'][1]  # Pièces max
        ...
        payload.update(self.parameters['leboncoin'])

        # Token de l'application Android Leboncoin
        token = "app_id=leboncoin_android&key=d2c84cdd525dddd7cbcc0d0a86609982c2c59e22eb01ee4202245" \
                "b7b187f49f1546e5f027d48b8d130d9aa918b29e991c029f732f4f8930fc56dbea67c5118ce"

        request = self.request(method="POST",
                               url="https://mobile.leboncoin.fr/templates/api/list.json",
                               params=payload,
                               data=token)
        data = request.json()
        ...
        for ad in data['ads']:
            ...
            _request = self.request(method="POST",
